...
#ifndef _REGISTER_CLASS_20240506_H 
#define _REGISTER_CLASS_20240506_H 1
...
// Set the constructor for the given type with namespace
template<class Cls, typename... Ts>
void dukglue_register_constructor_ns(duk_context* ctx, const char* ns, const char* name)
{
...
// creates the c'tor in a object with a prop with var_args (used as export param in duk mod_search)
template<class Cls>
void dukglue_register_constructor_managed_obj_varargs(duk_context* ctx, const char* name)
{
    duk_idx_t obj_idx = duk_push_object(ctx);
    duk_push_string(ctx, name);
    
    duk_c_function constructor_func = dukglue::detail::call_native_constructor_varargs<true, Cls>;
    duk_c_function finalizer_func = dukglue::detail::managed_finalizer<Cls>;
    
    duk_push_c_function(ctx, constructor_func, DUK_VARARGS);
...
// creates the c'tor in a object with a prop (used as export param in duk mod_search)
template<class Cls, typename... Ts>
void dukglue_register_constructor_managed_obj(duk_context* ctx, const char* name)
{
    duk_idx_t obj_idx = duk_push_object(ctx);
    duk_push_string(ctx, name);
...
template<class Cls, typename... Ts>
void dukglue_register_constructor_managed_ns(duk_context* ctx, const char* ns, const char* name)
{
...
void dukglue_register_method_varargs(duk_context* ctx,
    typename std::conditional<isConst, duk_ret_t(Cls::*)(duk_context*) const, 
    duk_ret_t(Cls::*)(duk_context*)>::type method,
    const char* name)
{
    using namespace dukglue::detail;
