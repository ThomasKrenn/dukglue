...
#ifndef _PUBLIC_UTIL_20240506_H
#define _PUBLIC_UTIL_20240506_H 1
...
//
// duk_pcompile() catches errors related to compilation (such as syntax errors in the source). 
// A zero return value indicates success and the compiled function is left on the stack top.
// A non-zero return value indicates an error, and the error is left on the stack top.
//
template <typename RetT>
typename std::enable_if<std::is_void<RetT>::value, RetT>::type dukglue_pcompile(duk_context* ctx, const char* file_name, const char* str)
{
   int prev_top = duk_get_top(ctx);
   ...
   int rc = duk_pcompile(ctx, 0);
   if (rc != 0) {
      // const char * p = duk_safe_to_string(ctx, -1);
      throw DukErrorException(ctx, rc);
   }
   else {
      rc = duk_pcall(ctx, 0);      // [ func ] -> [ result ]
      if (rc != 0) {
         // const char * p = duk_safe_to_string(ctx, -1);
         throw DukErrorException(ctx, rc);
      }
...
// 
// returns if object has callable methods with name: 'method_name'
//
template <typename ObjT>
bool dukglue_has_method(duk_context* ctx, const ObjT& obj, const char* method_name)
{
   dukglue_push(ctx, obj);
   duk_get_prop_string(ctx, -1, method_name);
