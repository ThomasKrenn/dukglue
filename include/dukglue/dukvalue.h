...
#ifndef _DUKVALUE_20240506_H
#define _DUKVALUE_20240506_H 1
...
public:
   ...
   inline DukValue() : mContext(nullptr), mType(UNDEFINED), mRefCount(nullptr) {}
...
public:
   ...
   bool is_bool() const {
      push();
      bool t_bool = duk_is_boolean(mContext, -1);
      duk_pop(mContext);
      return t_bool;
   }
   ...
   bool is_number() {
      push();
      bool t_number = duk_is_number(mContext, -1);
      duk_pop(mContext);
      return t_number;
   }
   ...
   std::vector<char> serialize() const {
      ...
      *((Type*)buff.data()) = mType;
      ...
      case STRING:
      {
         ...
         uint32_t len = static_cast<uint32_t>(mString.length());
...
private:
   ...
   static duk_uint_t stash_ref(duk_context* ctx, duk_idx_t idx)
   {
      ...
      if (next_free_idx == 0) {
         ...
         assert(len < DUK_UINT_MAX);
         next_free_idx = static_cast<duk_uarridx_t>(len);
      }
      else {
